...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awrecurse
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awrecurse-*.whl dist/*.tar.gz ...]
...
#: scans. __init__ is the entry point; cli is the command-line interface; engine
#: and client are core functionality.
_KEYSTONES = ("awrecurse/__init__.py", "awrecurse/cli.py", "awrecurse/engine.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awrecurse-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awrecurse/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awrecurse/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awrecurse/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awrecurse/cli.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awrecurse/y.py": "from awrecurse.client import call\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awrecurse-0.0.0.tar.gz"
